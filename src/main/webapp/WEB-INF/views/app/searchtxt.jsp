...
<body>
		...
		<div class="title">
			<span class="mui-action-back mui-icon mui-icon-back"></span>search：${catalog_name}
		...
		<c:if test="${result.pagenum>1}">
			<a href="http://www.sino-sources.com/app/search/${catalog_name}?topagenum=${result.prePagenum}"  class="mui-media-body mui-pull-left">上一页</a>
		</c:if>
		<c:if test="${result.pagenum<result.allpage}">
			<a href="http://www.sino-sources.com/app/search/${catalog_name}?topagenum=${result.nextPagenum}" class="mui-media-body mui-pull-right">下一页</a>
		</c:if>
		</li>
		</ul>
