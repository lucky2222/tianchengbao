$(function() {
	...
	$("#editContainerform").validate({
		...
		submitHandler : function(form) {
			...
			$.ajax({
				type : "POST",
				url : $.getContextPath()
						+ "/orderdetail/checkeditordercontainer",
				async : false,
				data : {
					content : JSON.stringify($("#editContainerform")
							.serializeJson()),
				...
				success : function(result) {
					if (result.substr(0, 1) == '0') {
						$.ajax({
									type : "POST",
									url : $.getContextPath()
											+ "/orderdetail/editordercontainer",
									async : false,
									data : {
										content : JSON
												.stringify($("#editContainerform")
														.serializeJson()),
										orderid : $("#orderid").text()
									},
									success : function(data) {
	...
	$('#editContainer').on('hide.bs.modal', function(event) {
				$("#editContainer-body").empty();
			});
});