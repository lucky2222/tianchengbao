$(function() {
	...
	function buttoninit()
	{
		...
		{
			$(".detailsection table tbody").closest("section").find("#receiveSaveFee").show();
			$(".detailsection table tbody").closest("section").find("#addreceivefeebtn").show();
			return false;
		}
		//无更新、无新增，展示新增按钮
		...
		{
			$(".detailsection table tbody").closest("section").find("#preinvoiceprimary").show();
		}
	...
	function orderfeeinit()
	{
		...
		$(".detailsection table tbody tr").click(function(){
	...
	function getJsonFromEdit(list,state)
	{
		...
		$("#"+list+" tbody tr[state='"+state+"']").each(function(){
			...
			comment.praccountsname = $(this).find("[id$='_praccountsid_show']").val();
	 ...
	 */
		...
		function readToEdit(currtext,modifyitem,feetype)
		{
			...
			*/
			...
			$(modifyitem).eq(7).find("#mod_praccountsid_show").val(currtext.eq(7).html());
			$(modifyitem).eq(7).find("#mod_praccountsid").val(currtext.closest("tr").attr("praccountsid"));
		 ...
		 * 监听费用确认按钮
		 */
		$("#alffeeconfirmbtn").click(function(){
			var list = $(this).attr("listname");
			$.post(
					$.getContextPath()
							+ "/orderfee/confirmallfeeitem",
							$("#allfeeconfirmform").serialize(),
					function(result) {
						var obj = jQuery.parseJSON(result);
						if(obj.resultflag)
						{
